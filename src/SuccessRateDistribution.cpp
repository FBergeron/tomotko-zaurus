...
void SuccessRateDistribution::paintEvent( QPaintEvent* ) {
    ...
    QString averageString = tr( "SuccessRateAverage" ).arg( successRateValueCount > 0 ? QString::number( successRateAverage ) : QObject::tr( "N/A" ) );
    p.drawText( size().width() - p.fontMetrics().width( averageString ) - margin, y, averageString );

    y += fm.height();
    
    QString standardDeviationString = tr( "SuccessRateStandardDeviation" ).arg( successRateValueCount > 0 ? QString::number( successRateStandardDeviation ) : QObject::tr( "N/A" ) );
