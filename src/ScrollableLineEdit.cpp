...
**
** Implementation of ScrollableLineEdit widget class
**
** Created : 941011
**
...
struct ScrollableLineEditUndoItem
{
    ScrollableLineEditUndoItem(){pos=0;};
    ScrollableLineEditUndoItem( const QString& s, int p )
    : str(s),pos(p){}
#if defined(Q_FULL_TEMPLATE_INSTANTIATION)
    bool operator==( const ScrollableLineEditUndoItem& ) const { return FALSE; }
#endif
    QString str;
    int pos;
};
...
struct ScrollableLineEditPrivate {
    ScrollableLineEditPrivate( ScrollableLineEdit * l ):
    frame(TRUE), mode(ScrollableLineEdit::Normal),
    readonly(FALSE), validator( 0 ),
    pm(0), pmDirty( TRUE ),
    blinkTimer( l, "ScrollableLineEdit blink timer" ),
    dragTimer( l, "ScrollableLineEdit drag timer" ),
    ...
    bool frame;
    ScrollableLineEdit::EchoMode mode;
    bool readonly;
    ...
    bool inDoubleClick;
    bool offsetDirty;
    QValueList<ScrollableLineEditUndoItem> undoList;
    QValueList<ScrollableLineEditUndoItem> redoList;
    bool undo;
    bool needundo;
...
// REVISED: arnt
/*!
  \class ScrollableLineEdit qlineedit.h

  \brief The ScrollableLineEdit widget is a one-line text editor.
  ...
  The default ScrollableLineEdit object has its own frame as specified by the
...
/*! \enum ScrollableLineEdit::EchoMode

  This enum type describes how ScrollableLineEdit displays its
...
/*!
  \fn void ScrollableLineEdit::textChanged( const QString& )
...
ScrollableLineEdit::ScrollableLineEdit( QWidget *parent, const char *name )
...
void ScrollableLineEdit::init()
{
    d = new ScrollableLineEditPrivate( this );
...
void ScrollableLineEdit::setText( const QString &text )
{
...
void ScrollableLineEdit::selectAll()
{
...
void ScrollableLineEdit::deselect()
{
...
void ScrollableLineEdit::setMaxLength( int m )
{
    maxLen = m;
    markAnchor = 0;
    markDrag = 0;
...
/*!
  \fn void  ScrollableLineEdit::returnPressed()
...
void ScrollableLineEdit::keyPressEvent( QKeyEvent *e )
{
    ...
    case Key_K:
        ...
        if ( d->undoList.isEmpty() || d->undoList.last().str != tbuf ) {
            d->undoList += ScrollableLineEditUndoItem(tbuf, cursorPos );
            d->redoList.clear();
        }
...
void ScrollableLineEdit::focusInEvent( QFocusEvent * e)
{
    d->pmDirty = TRUE;
    cursorOn = FALSE;
    blinkOn();
...
void ScrollableLineEdit::focusOutEvent( QFocusEvent * e )
{
...
/*!\reimp
*/
void ScrollableLineEdit::leaveEvent( QEvent * )
{
}
...
void ScrollableLineEdit::paintEvent( QPaintEvent *e )
{
    if ( d->offsetDirty )
    updateOffset();
    if ( !d->pm || d->pmDirty ) {
...
void ScrollableLineEdit::resizeEvent( QResizeEvent * )
{
    delete d->pm;
    d->pm = 0;
    offset = 0;
...
/*! \reimp
*/
bool ScrollableLineEdit::event( QEvent * e )
{
...
/*! \reimp
*/
void ScrollableLineEdit::mousePressEvent( QMouseEvent *e )
{
...
//void ScrollableLineEdit::doDrag()
//{
//    d->dnd_primed = FALSE;
...
/*!\reimp
*/
void ScrollableLineEdit::mouseMoveEvent( QMouseEvent *e )
{
//#ifndef QT_NO_DRAGANDDROP
...
/*!\reimp
*/
void ScrollableLineEdit::mouseReleaseEvent( QMouseEvent * e )
{
    dragScrolling = FALSE;
    d->dnd_primed = FALSE;
    if ( d->dndTimer.isActive() ) {
...
/*!\reimp
*/
void ScrollableLineEdit::mouseDoubleClickEvent( QMouseEvent * )
{
...
void ScrollableLineEdit::cursorLeft( bool mark, int steps )
{
...
void ScrollableLineEdit::cursorRight( bool mark, int steps )
{
...
void ScrollableLineEdit::newMark( int pos, bool /*c*/ )
{
...
void ScrollableLineEdit::markWord( int pos )
{
    int i = pos - 1;
...
void ScrollableLineEdit::clipboardChanged()
{
#if defined(_WS_X11_)
...
int ScrollableLineEdit::lastCharVisible() const
{
...
int ScrollableLineEdit::minMark() const
{
    return markAnchor < markDrag ? markAnchor : markDrag;
}


int ScrollableLineEdit::maxMark() const
{
    return markAnchor > markDrag ? markAnchor : markDrag;
}
...
void ScrollableLineEdit::setFrame( bool enable )
{
    if ( d->frame == enable )
    return;
...
int ScrollableLineEdit::frameW() const
{
...
void ScrollableLineEdit::setEchoMode( EchoMode mode )
{
    if ( d->mode == mode )
    return;
...
ScrollableLineEdit::EchoMode ScrollableLineEdit::echoMode() const
{
    return d->mode;
}
...
void ScrollableLineEdit::setValidator( const QValidator * v )
{
    d->validator = v;
}
...
void ScrollableLineEdit::clearValidator()
{
    setValidator( 0 );
}
...
/*! \reimp
*/
void ScrollableLineEdit::dragEnterEvent( QDragEnterEvent *e )
{
...
/*!\reimp
*/
void ScrollableLineEdit::dropEvent( QDropEvent *e )
{
    QString str;
...
void ScrollableLineEdit::blinkSlot()
{
...
void ScrollableLineEdit::dragScrollSlot()
{
...
bool ScrollableLineEdit::validateAndSet( const QString &newText, int newPos,
...
void ScrollableLineEdit::insert( const QString &newText )
{
    QString t( newText );
...
void ScrollableLineEdit::repaintArea( int from, int to )
{
...
void ScrollableLineEdit::setEnabled( bool e )
{
    d->pmDirty = TRUE;
...
void ScrollableLineEdit::setFont( const QFont & f )
{
    d->pmDirty     = TRUE;
    d->offsetDirty = TRUE;
    QWidget::setFont( f );
...
void ScrollableLineEdit::setSelection( int start, int length )
{
    int b, e;
...
void ScrollableLineEdit::setCursorPosition( int newPos )
{
    if ( newPos == cursorPos )
    return;
...
void ScrollableLineEdit::setPalette( const QPalette & p )
{
    d->pmDirty = TRUE;
...
is never read by ScrollableLineEdit, and is changed to TRUE whenever the user
...
of the ScrollableLineEdit (or the last call to either setText() or setEdited( FALSE ),
...
void ScrollableLineEdit::updateOffset()
...
int ScrollableLineEdit::xPosToCursorPos( int goalx ) const
{
    int x1, x2;
    x1 = offset;
    int i = 0;
...
void ScrollableLineEdit::blinkOn()
{
    if ( !hasFocus() )
    return;
...
void ScrollableLineEdit::makePixmap() const
{
    if ( d->pm )
    return;
...
void ScrollableLineEdit::undoInternal()
{
    if ( d->undoList.isEmpty() )
    return;
    d->undo = FALSE;

    d->redoList += ScrollableLineEditUndoItem(tbuf, cursorPos );
...
void ScrollableLineEdit::redoInternal()
{
...
bool ScrollableLineEdit::getSelection( int *start, int *end )
{
    if( !hasMarkedText() )
    return false;
...
void ScrollableLineEdit::setPasswordChar( QChar c )
{
    d->passwordChar = c;
}

QChar ScrollableLineEdit::passwordChar() const
{
...
// Added methods for toMOTko.
void ScrollableLineEdit::scrollLeft() {
    int firstCharPos = xPosToCursorPos( 0 );
    setCursorPosition( firstCharPos /*- 1*/ );
}

void ScrollableLineEdit::scrollRight() {
    int lastCharPos = lastCharVisible();
    setCursorPosition( lastCharPos /*+ 1*/ );
}

void ScrollableLineEdit::keyReleaseEvent( QKeyEvent *e ) {
    if ( e->state() & ControlButton ) {
        switch ( e->key() ) {
#ifndef QT_NO_CLIPBOARD
            case Key_C:
