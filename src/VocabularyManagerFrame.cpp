...
void VocabularyManagerFrame::exportVocabulary( Vocabulary* vocab ) {
    ...
    ExportDialog msgBox( tr( "Export..." ), translationLanguages, 
        ExportDialog::selectionModeAllLanguages, controller, this );
    msgBox.setMaximumHeight( size().height() - 40 );
    msgBox.setMaximumWidth( size().width() - 40 );
    int response = msgBox.exec();
    if( !response )
    ...
    if( !msgBox.getSelectedLanguages().isEmpty() )
        languagesToExport = msgBox.getSelectedLanguages();
    exportStats = msgBox.getIncludeStats();
