...
glossaries.keys.sort().each {
    ...
    outputFile = glossary.title + '.zip'
    
    Zip::ZipOutputStream::open( outputFile ) {
        | io |
        ...
        io.write "<glossary id=\"0\" name=\"" + glossary.title + "\">"
    
        glossary.terms.keys().sort().each {
            | termKey |
 
            term = glossary.terms[ termKey ]

            puts term.id
            puts term.comments
            puts term.translations
            trans_en = term.translations[ 'en' ]
            trans_ja = term.translations[ 'ja' ]

            kanji = trans_ja.word
            furigana = trans_ja.alt
            english = trans_en.word
