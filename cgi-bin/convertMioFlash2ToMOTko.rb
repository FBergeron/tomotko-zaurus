...
#
# The code that follows is the code of rubyzip.
# For some unknown reason, I cannot simply execute:
#
# require "zip/zip"
#
# (or any other require statement refering to personal
# files for that matter), so I had to include it into
...
module Zip
  ...
  RUNNING_ON_WINDOWS = /mswin33|cygwin|mingw|bccwin/ =~ RUBY_PLATFORM
...
#
# Here is the converter's code.
#

outputFile = Tempfile.new( 'glossary' )

Zip::ZipOutputStream::open( outputFile.path ) {
    | io |

    io.put_next_entry( "v-1/vocab-1.xml" )
    ...
    io.write "<glossary id=\"0\" name=\"" + glossaryName + "\">"
    ...
    while( line = gets ) 
        ...
        if( kanji || furigana || english )
            io.write "  <term id=\"" + termId.to_s + "\">"
            if( english )
                io.write "    <trans lang=\"en\">"
                io.write "      <word>" + english + "</word>"
                io.write "    </trans>"
            end
            if( kanji || furigana )
                io.write "    <trans lang=\"ja\">"
                io.write "      <word>" + kanji + "</word>" if( kanji )
                io.write "      <alt>" + furigana + "</alt>" if( furigana )
            end
            io.write "    </trans>"
            io.write "    <comment languages=\"en-ja\">" + comment + "</comment>" if( comment )
            io.write "  </term>"
...
$stdout.write IO.read( outputFile.path )
